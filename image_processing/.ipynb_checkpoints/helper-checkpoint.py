...
def z_pad(image):
    ...
    print('z_pad',depth,height,width)
...
def xy_pad(image):
    ...
    if height > 256:
        if height % 2 == 0:
            image = image[:][diff_h:height-diff_h][:]#image[:,diff_h:height-diff_h,:]
        else:
            image = image[:][diff_h+1:height-diff_h][:]#image[:,diff_h+1:height-diff_h,:]
    else:
        if height % 2 == 0:
